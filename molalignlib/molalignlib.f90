...
subroutine remap_atoms( &
   ...
   type(Partition) :: atomequiv0, atomequiv1
...
!   integer :: nbond1, bonds1(2, maxcoord*mol1%natom)
   ...
   call atomequiv0%set_partition(mol0%get_natomequiv(), mol0%get_atomequividcs())
   call atomequiv1%set_partition(mol1%get_natomequiv(), mol1%get_atomequividcs())
   ...
   if (any(mol0%get_atomelnums(atomequiv0) /= mol1%get_atomelnums(atomequiv1))) then
      write (stderr, '(a)') 'Error: The molecules are not isomers'
      error = 1
      return
   end if
   ...
   if (any(mol0%get_atomtypeidcs(atomequiv0) /= mol1%get_atomtypeidcs(atomequiv1))) then
   ...
   if (any(abs(mol0%get_atomweights(atomequiv0) - mol1%get_atomweights(atomequiv1)) > 1.E-6)) then
   ...
   ! Optimize assignment to minimize the AdjD and RMSD
    ...
    call optimize_mapping(mol0, mol1, atomequiv0, atomequiv1, maplist, countlist, nrec)

   ! Debond reactive sites and reoptimize assignment

!   if (reac_flag) then
!      call find_reactive_sites(mol0, mol1, atomequiv0, atomequiv1, maplist(:, 1))
...
!!   call writemol2(99, 'coords1', natom1, workznums1(mapping), workcoords1(:, mapping), nbond1, bonds1)
   ...
   indexlist1 = atomequiv1%get_atomidcs()
   indexmap0 = atomequiv0%get_atomidxmap()
