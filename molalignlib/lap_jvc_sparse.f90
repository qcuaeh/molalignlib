...
!-----------------------------------------------------------------------------
   ...
   integer, intent(in) :: kk(sz), first(n+1)
   integer, intent(out) :: rowsol(n)
   ...
   ! Working arrays
   integer :: x(n), y(n), free(n), todo(n), lab(n), ok(n)
   integer :: xinv(n)
   real(rk) :: u(n), v(n), d(n)
   
   ! Local variables
   integer :: i, jp, j0p, j1p, i0, t, tp, h, lp, l0p, tel
   integer :: l, l0, td1
...
!-----------------------------------------------------------------------------
   integer, intent(in) :: n, sz, l
   real(rk), intent(in) :: cc(sz), infValue, resolution
   integer, intent(in) :: kk(sz), first(n+1), free(n)
   integer, intent(inout) :: x(n), y(n), lab(n), todo(n), ok(n), td1
