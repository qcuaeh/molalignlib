...
subroutine recompute_mna_partition(atoms1, atoms2, link)
...
subroutine distribute_part_items(atoms1, atoms2, branch)
...
!      call print_link_itemdir(link)
      call recompute_mna_partition(atoms1, atoms2, link)
      link_idx = link_idx + 1
      link => link%next_link
   end do
end subroutine
...
recursive subroutine distribute_items(atoms1, atoms2, branch)
   type(atom_t), dimension(:), intent(in) :: atoms1, atoms2
   type(assigntree_node_t), pointer, intent(inout) :: branch
...
!      write (stderr,'(A,1X,A)') 'Split Part', address(child_branch%split_part)
      ...
      call distribute_part_items(atoms1, atoms2, child_branch)

      ! Recursively process this child's descendants (depth-first)
      call distribute_items(atoms1, atoms2, child_branch)
