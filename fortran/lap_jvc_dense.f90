...
!---------------------------------------------------------------------------------------
   ...
   integer(ik), intent(out) :: rowsol(:)
   real(rk), intent(out) :: lapcost
   
   ! Local variables
   integer(ik) :: i, j, j1, j2, k, f
   integer(ik) :: imin, numfree, prvnumfree, i0, freerow
   integer(ik) :: last, low, up, endofpath, loopcnt
   real(rk) :: dmin, h, umin, usubmin, v2
   real(rk), parameter :: resolution = 1.0e-10_rk
   logical(lk) :: unassignedfound
   
   ! Working arrays
   real(rk), allocatable :: v(:), d(:), u(:)
   integer(ik), allocatable :: free(:), collist(:), matches(:)
   integer(ik), allocatable :: pred(:), colsol(:)
