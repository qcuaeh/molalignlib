program test_dict_part2a
    ...
    integer, parameter :: TEST_KEY_LENGTH = 3
    ...
    do while (.true.)
        ...
        call d%put(key, value)
        expected_dict_size = expected_dict_size + 1

        if (d%exists(key) .and. d%get(key) == value) then
    ...
    print '(A,I0)', "Total combinations inserted: ", test_count
    ...
    do i = 0, d%num_collision_levels
        print '(A,I2,A,I10,A,F6.2,A)', "  Level ", i, " occupations: ", d%occupations(i), &
            " (", real(d%occupations(i)) / real(d%actual_capacity / (2**i)) * 100, "%)"
    end do
