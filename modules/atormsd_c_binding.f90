...
subroutine atormsd_calculate(c_file1, c_file2, &
                              c_align, c_remap, c_heavy, c_mass, &
                              c_mirror, c_label, c_mapping, &
                              c_stats, c_random, &
                              c_prune_tol, c_ato_thres, c_max_trials, c_num_records, &
   ...
   integer(c_int), intent(in), value :: c_mapping
   integer(c_int), intent(in), value :: c_stats
   integer(c_int), intent(in), value :: c_random

   ! Numeric and integer parameters
   real(c_double), intent(in), value :: c_prune_tol
   integer(c_int), intent(in), value :: c_ato_thres
   ...
   mapping_flag = (c_mapping /= 0)
   stats_flag = (c_stats /= 0)
   random_flag = (c_random /= 0)
   ...
   ! Set pruning procedure
   if (c_prune_tol < 0.0_c_double) then
      prune_procedure => prune_none
   else
      prune_procedure => prune_rd
      prune_tol = real(c_prune_tol, rk)
   end if
   ...
   if (align_flag) then
      ...
      if (remap_flag) then
         ...
         if (any(atomtypes%itemdir1 /= atomtypes%itemdir2)) then
            c_error_code = 2  ! Atom types do not match
            return
         end if
